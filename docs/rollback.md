...
Rollback now assumes the consolidated `platform-shared` Droplet is the only active production host for DotDev, Prefect, and Airflow. The old App Platform app, old Prefect Droplet, and retired 4 GiB platform Droplet have already been deleted.
...
3. Verify `raniendu.dev`, `prefect.raniendu.dev`, and `flow.raniendu.dev` against the restored target.
4. Fix the new stack and repeat public verification before trying another cutover.

## Data Considerations

Production currently uses one shared Postgres container with separate `prefect` and `airflow` databases. After the old 4 GiB Droplet was decommissioned, rollback requires restoring from the latest Droplet backup/snapshot or app-level database backup.
