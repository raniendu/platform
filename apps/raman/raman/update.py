"""Self-update for the raman CLI.

Mirrors what ``apps/raman/install.sh`` does at install time: locates the
on-disk source checkout, fast-forwards it, and reinstalls the ``raman`` uv
tool from that checkout.

This module is import-light on purpose — no agent, pydantic-ai, or settings
imports — so ``raman update`` runs even on a machine whose model provider
config is broken.
"""
...
DEFAULT_INSTALL_DIR = Path.home() / ".local" / "share" / "raman"
...
def write_metadata(data: dict[str, Any], path: Path | None = None) -> Path:
    ...
    lines = ["# Written by raman install/update — do not edit by hand.\n"]
...
def resolve_source(explicit: str | None = None) -> Path:
    ...
    Priority: --source flag > metadata file > $RAMAN_INSTALL_DIR > default.
    """
    ...
    env_dir = os.environ.get("RAMAN_INSTALL_DIR")
    if env_dir:
    ...
    raise UpdateError(
        "No raman checkout found. Looked in:\n"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="raman update",
        description="Pull the latest raman source and reinstall the CLI.",
        epilog=("Rollback: git -C <source> reset --hard <good-sha> && raman update"),
    )
    parser.add_argument(
        "--source",
        default=None,
        help=(
            "Path to the raman repo checkout to update. Overrides the install "
            "metadata and $RAMAN_INSTALL_DIR."
        ),
    )
    parser.add_argument(
        "--check",
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if not (raman_dir / "pyproject.toml").is_file():
            print(
                f"Expected {raman_dir}/pyproject.toml; aborting reinstall.",
                file=sys.stderr,
            )
            return 1
