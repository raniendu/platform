...
- When `DEPLOY_RAMAN=true`, the GitHub `production` environment includes `DO_INFERENCE_API_KEY` and `PARALLEL_API_KEY` secrets, and `PLATFORM_ENV_FILE` includes the Raman Telegram env names referenced by `apps/raman/spec/telegram.toml` such as `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_ALLOWED_CHAT_IDS`, and `TELEGRAM_BOT_USERNAME`. `PARALLEL_API_KEY` is required because the current Raman spec enables `web_search`.
