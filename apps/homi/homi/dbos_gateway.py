...
INBOUND_QUEUE = Queue("homi-inbound", concurrency=1, polling_interval_sec=0.1)
OUTBOUND_QUEUE = Queue("homi-outbound", concurrency=1, polling_interval_sec=0.1)
...
def configure_dbos(settings: HomiSettings) -> None:
    global _configured
    if _configured:
        return
...
def launch_dbos(settings: HomiSettings) -> None:
...
@DBOS.workflow(name="homi_deliver_reply")
...
@DBOS.step(name="homi_send_telegram_reply", retries_allowed=True, max_attempts=3)
