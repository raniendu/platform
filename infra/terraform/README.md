...
The desired steady-state target is `s-1vcpu-2gb`, which is the current production size after the completed May 2026 smaller-Droplet migration. The Droplet resource sets `resize_disk = false`, enables DigitalOcean monitoring, sets `prevent_destroy = true`, and ignores size/networking/SSH/user-data drift so routine deploys adopt the existing Droplet instead of replacing it for provider default drift. The deploy workflow imports an existing `platform-shared` Droplet before applying so a missing GitHub-side state file does not create a duplicate Droplet. The output `droplet_ip` is the value to use for Squarespace A records after a recreate/disaster-recovery event.
