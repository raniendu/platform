...
prefect_config_strategy = st.builds(
    PrefectConfig,
    api_url=st.text(min_size=1, max_size=100).map(
        lambda s: f"http://{s.replace(' ', '')}/api"
    ),
    ui_url=st.text(min_size=1, max_size=100).map(
        lambda s: f"http://{s.replace(' ', '')}"
    ),
)
...
environment_config_strategy = st.builds(
    ...
    domain=st.one_of(
        st.none(), st.text(min_size=1, max_size=100).filter(lambda s: s.strip() != "")
    ),
)
