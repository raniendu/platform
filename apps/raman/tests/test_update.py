...
from raman import update as update_module


def _redirect_config_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    monkeypatch.delenv("RAMAN_INSTALL_DIR", raising=False)
    return tmp_path / "raman" / "install.toml"
...
def test_metadata_roundtrip(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    update_module.write_metadata(
        {
            "source_dir": "/tmp/some path/raman",
    ...
    assert loaded["source_dir"] == "/tmp/some path/raman"
...
def test_resolve_source_falls_back_to_env(
    ...
    monkeypatch.setenv("RAMAN_INSTALL_DIR", str(env_src))
...
def _wire_source(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> tuple[Path, Path]:
    ...
    meta_file = _redirect_config_dir(monkeypatch, tmp_path)
    source = tmp_path / "raman-src"
    (source / ".git").mkdir(parents=True)
    raman_dir = source / "apps" / "raman"
    raman_dir.mkdir(parents=True)
    (raman_dir / "pyproject.toml").write_text("# stub\n", encoding="utf-8")
...
def test_run_aborts_on_dirty_tree(
    ...
    git = GitFake({("status", "--porcelain"): " M raman/cli.py"})
    monkeypatch.setattr(update_module, "_git", git)
