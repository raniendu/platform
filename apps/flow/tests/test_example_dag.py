...
def test_dag_has_required_attributes():
    ...
    ), f"Expected dag_id 'example_dag', got '{dag.dag_id}'"
...
def test_dag_task_dependencies():
    ...
    assert (
        hello_task in start_task.downstream_list
    ), "hello_task should be downstream of start"
    assert (
        end_task in hello_task.downstream_list
    ), "end should be downstream of hello_task"

    # Verify upstream dependencies
    assert (
        start_task in hello_task.upstream_list
    ), "start should be upstream of hello_task"
