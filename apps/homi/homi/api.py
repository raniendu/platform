...
from homi.agent import HomiAgentRunner, build_agent_runner
from homi.dbos_gateway import EventDispatcher, launch_dbos, shutdown_dbos
from homi.gateway import InboundMessage, ThreadStore
from homi.settings import HomiSettings
from homi.spec import load_spec
from homi.telegram import TelegramAdapter
...
_settings: HomiSettings | None = None
_agents: dict[str, HomiAgentRunner] = {}
_store: ThreadStore | None = None
_dispatcher: EventDispatcher | None = None
_telegram_adapter: TelegramAdapter | None = None


def _get_settings() -> HomiSettings:
    global _settings
    if _settings is None:
        _settings = HomiSettings()
    return _settings


def _get_agent(name: str) -> HomiAgentRunner:
    if name not in _agents:
        settings = _get_settings()
        spec = load_spec(name, settings.spec_root)
        _agents[name] = build_agent_runner(spec=spec, settings=settings)
    return _agents[name]
...
def _get_store() -> ThreadStore:
    global _store
    if _store is None:
        _store = ThreadStore(_get_settings().homi_db_path)
    return _store
...
app = FastAPI(title="homi", lifespan=lifespan)
...
async def chat(req: ChatRequest) -> ChatResponse:
    ...
    result = await agent.run(
        ...
        conversation_id=f"http:{name}",
    )
    return ChatResponse(agent=name, output=result.output)
...
async def telegram_webhook(
    ...
    if not settings.telegram_webhook_secret:
        raise HTTPException(
            status_code=503,
            detail="HOMI_TELEGRAM_WEBHOOK_SECRET is not configured",
        )
...
def run() -> None:
    ...
    uvicorn.run("homi.api:app", host="127.0.0.1", port=8000, reload=False)
