...
droplet_name_strategy = st.from_regex(r"[a-z0-9][a-z0-9-]{1,28}[a-z0-9]", fullmatch=True)
...
@settings(max_examples=10, deadline=None)  # Reduced examples, no deadline for terraform operations
    ...
    **Validates: Requirements 3.1, 3.2**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        prefect_resource = next(
            (r for r in resources if r.get("address") == "null_resource.prefect_droplet"),
            None
        )
        assert prefect_resource is not None, "State should contain prefect_droplet resource"
