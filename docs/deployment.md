...
- When `DEPLOY_RAMAN=true`, the GitHub `production` environment includes `DO_INFERENCE_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_ALLOWED_CHAT_IDS`, and `PARALLEL_API_KEY` secrets. `PARALLEL_API_KEY` is required because the current Raman spec enables `web_search`.
