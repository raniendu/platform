...
from homi.gateway import EnqueuedEvent, ThreadStore
from homi.settings import HomiSettings
from homi.telegram import TelegramAdapter, format_for_telegram
...
def make_settings(**overrides):
    ...
    return HomiSettings(_env_file=None, **values)
...
async def test_telegram_reset_command_clears_history(tmp_path):
    store = ThreadStore(tmp_path / "homi.sqlite3")
    store.set_history("telegram", "123", agent_name="homi", message_history_json=b"[]")
    sent = []
...
async def test_telegram_agent_command_persists_existing_spec(tmp_path):
    ...
    result = await adapter.handle_update(text_update("/agent homi"))
    ...
    assert sent == [(123, chunk) for chunk in expected("Agent set to homi.")]
